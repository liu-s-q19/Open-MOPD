...
# Adaptive multi-node Qwen3 Open-MOPD launcher.
#
# This launcher keeps the Qwen3-4B model/data recipe from vanilla_mopd.sh and
# enables the Open-MOPD budget mechanisms: token-share balancing, gap-following
# allocation, and student-dependent reward refresh across inner updates.
#
# The default IF teacher is the Math teacher because no independent Qwen3 IF
# checkpoint is currently available. Override it with --if-teacher or
# IF_TEACHER_PATH when one is available. This is therefore an Open-MOPD-style
# Qwen3 run, not the official SmolLM3 exact reproduction.
#
...
IF_TEACHER_FALLBACK=0
if [[ -z "${IF_TEACHER_PATH:-}" ]]; then
    IF_TEACHER_PATH="${MATH_TEACHER_PATH}"
    IF_TEACHER_FALLBACK=1
fi
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-open_mopd}"
OUTPUT_DIR="${OUTPUT_DIR:-${OPEN_MOPD_ROOT}/outputs/open_mopd_qwen3/qwen3_4b}"
...
Default teacher mapping: [Math, Code, Math] -> [math, code, if].
Use --if-teacher or IF_TEACHER_PATH to replace the temporary IF fallback.
...
Options:
  ...
  --if-teacher PATH             override IF_TEACHER_PATH
...
if ((IF_TEACHER_FALLBACK)); then
    # A command-line --math-teacher override must also update the default IF
    # fallback; explicit --if-teacher has already cleared this flag.
    IF_TEACHER_PATH="$MATH_TEACHER_PATH"
fi
...
[[ "$TRAIN_BATCH_SIZE" =~ ^[1-9][0-9]*$ ]] || die "TRAIN_BATCH_SIZE must be positive: $TRAIN_BATCH_SIZE"
[[ "$PPO_MINI_BATCH_SIZE" =~ ^[1-9][0-9]*$ ]] || die "PPO_MINI_BATCH_SIZE must be positive: $PPO_MINI_BATCH_SIZE"
((TRAIN_BATCH_SIZE % PPO_MINI_BATCH_SIZE == 0)) || die \
    "TRAIN_BATCH_SIZE=$TRAIN_BATCH_SIZE must be divisible by PPO_MINI_BATCH_SIZE=$PPO_MINI_BATCH_SIZE"
INNER_UPDATES=$((TRAIN_BATCH_SIZE / PPO_MINI_BATCH_SIZE))
...
if ((RUN_TRAINING)); then
    ...
    [[ -d "$IF_TEACHER_PATH" ]] || die "IF teacher does not exist: $IF_TEACHER_PATH"
fi
...
echo "[open-mopd-qwen3] python=$PYTHON_BIN"
echo "[open-mopd-qwen3] patched_verl=${OPEN_MOPD_ROOT}/training/verl"
echo "[open-mopd-qwen3] git_commit=$GIT_COMMIT"
echo "[open-mopd-qwen3] node_rank=$NODE_RANK nnodes=$NNODES visible_gpus=$VISIBLE_GPUS_PER_NODE expected_gpus=$EXPECTED_GPUS"
echo "[open-mopd-qwen3] master=$MASTER_ADDR:$MASTER_PORT"
echo "[open-mopd-qwen3] ray_ports dashboard=$RAY_DASHBOARD_PORT dashboard_agent_grpc=$RAY_DASHBOARD_AGENT_GRPC_PORT dashboard_agent=$RAY_DASHBOARD_AGENT_LISTEN_PORT metrics=$RAY_METRICS_EXPORT_PORT runtime=$RAY_RUNTIME_ENV_AGENT_PORT workers=$RAY_MIN_WORKER_PORT-$RAY_MAX_WORKER_PORT"
echo "[open-mopd-qwen3] model=$MODEL_PATH"
echo "[open-mopd-qwen3] teachers=[math:$MATH_TEACHER_PATH,code:$CODE_TEACHER_PATH,if:$IF_TEACHER_PATH]"
echo "[open-mopd-qwen3] if_teacher_fallback=$IF_TEACHER_FALLBACK"
echo "[open-mopd-qwen3] open_mopd=share+gap+refresh target_shares=[math:1,code:1,if:1]"
echo "[open-mopd-qwen3] train=$TRAIN_FILE"
echo "[open-mopd-qwen3] val_files=$VAL_FILES"
echo "[open-mopd-qwen3] output=$OUTPUT_DIR checkpoint=$CKPT_DIR"
echo "[open-mopd-qwen3] open_mopd train_batch=$TRAIN_BATCH_SIZE n=$N_RESPONSES ppo_mini=$PPO_MINI_BATCH_SIZE ppo_epochs=1 inner_updates=$INNER_UPDATES top_k=$LOG_PROB_TOP_K prompt_len=$MAX_PROMPT_LENGTH response_len=$MAX_RESPONSE_LENGTH epochs=$TOTAL_EPOCHS steps=$TOTAL_TRAINING_STEPS"
...
TRAIN_CMD=(
    ...
    "+mt_opd.target_share_values=[1,1,1]"
    "+mt_opd.normalize_reward_scale=True"
...
# Teacher 1 is Code; teacher 2 is the configured IF teacher, defaulting to Math.
TRAIN_CMD+=(
    ...
    "+mt_reward_model_2.model.path=$IF_TEACHER_PATH"
...
printf '[open-mopd-qwen3] command:'
printf ' %q' "${TRAIN_CMD[@]}"
printf '\n'

if [[ "$RUN_TRAINING" != 1 ]]; then
    echo "[open-mopd-qwen3] dry-run only; pass --run or omit --dry-run to execute"
    exit 0
fi
...
if ((NODE_RANK == 0)); then
    ...
    until "$RAY_BIN" start --address="$MASTER_ADDR:$MASTER_PORT" "${RAY_PORT_ARGS[@]}" --block; do
        echo "[open-mopd-qwen3] worker $NODE_RANK could not connect to Ray head; retrying in 3s..."
        sleep 3
    done
fi
...
PY
        ...
        echo "[open-mopd-qwen3] [$ELAPSED/$MAX_WAIT_SECONDS] cluster=$CURRENT_GPUS/$EXPECTED_GPUS available=$AVAILABLE_GPUS/$EXPECTED_GPUS"
    ...
    echo "[open-mopd-qwen3] worker rank $NODE_RANK is attached to Ray; waiting for rank 0"
fi
